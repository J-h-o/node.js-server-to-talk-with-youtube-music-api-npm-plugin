...
const fs = require("fs");
const YoutubeMusicApi = require("./albumSearch/index.js");
...
const apiAlbum = new YoutubeMusicApi();
...
app.get("/album/:albumId", async (req, res) => {
  ...
  let album = await apiAlbum
    .initialize()
    // .then(() => apiAlbum.getAlbum(albumId));
    .then((info) => {
      apiAlbum.getAlbum(albumId).then((result) => {
        res.send(result);
      });
    });
  //res.send(album);
